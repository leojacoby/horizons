...
hangman.isGuessWrong = function(word, guess) {
  // YOUR CODE HERE
  if (guess.length > 1) {
    throw "Only 1 letter guesses please!"
  }
...
hangman.words = [ 'cat', 'doctor', 'dog', 'horizons', 'school', 'technology', 'baseball', 'elevation' ];
...
hangman.getGameStatus = function(word, guesses) {
  ...
  for (var i = 0; i < guesses.length; i++) {
    if (hangman.isGuessWrong(word, guesses[i])) {
      wrong++
    } else {
      right_guesses.push(guesses[i])
    }
  }

  if (wrong >= hangman.maxWrongGuesses) {
    return "lose"
  }

  var win = true
  for (var i = 0; i < word.length; i++) {
    if (!right_guesses.includes(word[i])) {
      win = false
    }
  }
...
hangman.makeGuess = function(letter) {
  // YOUR CODE HERE
  if (hangman.getGameStatus(hangman.game.word, hangman.game.guesses) !== 'in_progress') {
    throw "Looks like this...game...is...OVER!"
  }
  hangman.game.guesses.push(letter)
  hangman.isGuessWrong(hangman.game.word, letter)
  hangman.drawGame()
}
