...
towers.isValidBoard = function(board) {
  ...
  for (var i = 0; i < board.length; i++) {
    ...
    towers = towers + board[i].length

    current_tower = 3
    
    for (var x = 0; x < board[i].length; x++) {
      tower_nums.push(board[i][x])
      if (board[i][x] < current_tower) {
        current_tower = board[i][x]
      } else {
        return false
      }
    }
  ...
  if (towers !== 3 || !(tower_nums.includes(0) && tower_nums.includes(1) && tower_nums.includes(2))) {
    return false
...
towers.makeMove = function(fromTower, toTower) {
  var newBoard = towers.copyBoard(towers.game.board)
  if (newBoard[fromTower].length < 1) {
    throw "There isn't anything in that tower"
  }
  if (newBoard[fromTower][newBoard[fromTower].length - 1] > newBoard[toTower][newBoard[toTower].length - 1]) {
    throw "Invalid move"
  }
  if (!towers.isValidBoard(newBoard)) {
    throw "Uh oh.  In valid board!"
  }
  newBoard[toTower].push(newBoard[fromTower][newBoard[fromTower].length - 1])
  newBoard[fromTower].splice(-1, 1)

  towers.game.board = newBoard
